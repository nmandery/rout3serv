...
/// `IndexBlock` allows to store h3index in compressed form.
///
    ...
    T: Into<u64> + Copy,
{
    ...
    pub fn contains(&self, index: &T) -> Result<bool, Error> {
        ...
        let h3index_bytes = Into::<u64>::into(*index).to_le_bytes();
    ...
    pub const fn size_of_uncompressed(&self) -> usize {
        size_of::<Vec<u64>>() + size_of::<u64>() * self.len()
    }
...
mod tests {
    ...
    fn make_grid_disk(k: u32) -> Vec<CellIndex> {
        let idx = CellIndex::try_from(0x89283080ddbffff_u64).unwrap();
        idx.grid_disk(k)
    }

    fn grid_disk_indexblock_roundtrip(cells: Vec<CellIndex>) -> IndexBlock<CellIndex> {
    ...
    fn test_indexblock_from_iter() {
        let ib: IndexBlock<CellIndex> = IndexBlock::from_iter(make_grid_disk(3).iter());
        assert!(!ib.is_empty());
    }
    ...
    fn test_indexblock_contains() {
        ...
        let disk: Vec<_> = cell.grid_disk(8);
        let ring: Vec<_> = cell.grid_ring_fast(9).flatten().collect();
